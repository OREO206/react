...
export default function StateComponent() {
    ...
    const onCountAddHandler = () => {
        ...
        // 상태변경 함수를 통해서 함수를 변경 한다고 바로 적용되지 않음
        // 리렌더링된 후 상태변경 함수가 적용
        // setCount(count + 1);
        // console.log(count);
        // setCount(count + 1);
    ...
    // Input 요소에 값을 가져오고자 할 땐 onChange 이벤트의 .target.value로 가져옴
    ...
    const onAddListHandler = () => {
        // 상태가 배열 혹은 객체이면 요소를 각각에 대해 요소를 추가, 변경이 일어나도 상태가 변경된 것으로 인식하지 않음(실제 주소 변경 X)
    ...
    return (
        <>
            ...
            {/* Input이 만약 상태를 변경한다면 vlaue로 그 상태를 지정해야 불일치가 발생하지 않음 */}
